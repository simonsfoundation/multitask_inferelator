...
class TestAMuSRrunner(unittest.TestCase):
    ...
    def test_format_priors_noweight(self):
        runner = amusr.AMuSR_regression()
        tfs = ['tf1', 'tf2']
        priors = [pd.DataFrame([[0, 1], [1, 0]], index = ['gene1', 'gene2'], columns = tfs),
                  pd.DataFrame([[0, 0], [1, 0]], index = ['gene1', 'gene2'], columns = tfs)]
        gene1_prior = runner.format_prior(priors, 'gene1', tfs, [0, 1], 1)
        gene2_prior = runner.format_prior(priors, 'gene2', tfs, [0, 1], 1)
    ...
    def test_format_priors_pweight(self):
        ...
        priors = [pd.DataFrame([[0, 1], [1, 0]], index = ['gene1', 'gene2'], columns = tfs),
                  pd.DataFrame([[0, 0], [1, 0]], index = ['gene1', 'gene2'], columns = tfs)]
        gene1_prior = runner.format_prior(priors, 'gene1', tfs, [0, 1], 1.2)
        gene2_prior = runner.format_prior(priors, 'gene2', tfs, [0, 1], 1.2)
    ...
    def test_sum_squared_errors(self):
        ...
        self.assertEqual(amusr.sum_squared_errors(X, Y, W, 0), 0)
        self.assertEqual(amusr.sum_squared_errors(X, Y, W, 1), 27)

    def test_amusr_regression(self):

        runner = amusr.AMuSR_regression()
        des = [np.array([[1, 1, 3], [0, 0, 2], [0, 0, 1]]),
             np.array([[1, 1, 3], [0, 0, 2], [0, 0, 1]])]
        res = [np.array([1, 2, 3]).reshape(-1, 1),
        ...
        gene2_prior = runner.format_prior(priors, 'gene2', tfs, [0, 1], 1.)
        output = []
        output.append(amusr.run_regression_EBIC(des, res, ['tf1', 'tf2', 'tf3'], [0, 1], 'gene1', gene1_prior))
        output.append(amusr.run_regression_EBIC(des, res, ['tf1', 'tf2', 'tf3'], [0, 1], 'gene2', gene2_prior))
